...
async function main() {
  ...
  requestAnimationFrame(() =>
    requestAnimationFrame(() => {
      ...
      renderStars(brandData);

      // 페이지 2
      const { regionData } = data.page2;
      initGeoChart({ elementId: "storeGeoChart", regionData });

      const { xAxisData: xAxisData2, yAxisData: yAxisData2 } =
        data.page2.store_trend;
      initStoreCountChart({
      ...
      // 페이지 3
      const { xAxisData: xAxisData3, yAxisData: yAxisData3 } =
        data.page3.sales_trend;
      initSalesChart({
