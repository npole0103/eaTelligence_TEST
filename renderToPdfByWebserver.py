...
def renderingLogo(brnd_no: str):
    ...
    # HTML 파일 저장 (브랜드별)
    rendered_path = RESOURCE_PATH / f"{brnd_no}.html"
...
def render_with_js_and_export_pdf(brnd_no: str):
    # 브랜드별 HTML 생성
    html_path = renderingLogo(brnd_no)

    # 간이 웹 서버 시작
    server, thread = run_static_server(RESOURCE_PATH)

    OUTPUT_FILE_NAME = f"{brnd_no}.pdf"
    ...
    with sync_playwright() as p:
        ...
        page.goto(f"http://localhost:8000/resources/{brnd_no}.html")
